...
# ================== وب‌سرور کوچک (فقط برای Render Web Service) ==================
# Render برای Web Service انتظار داره برنامه یه پورت باز کنه، وگرنه سرویس رو
# ناسالم تشخیص میده. این سرور فقط یه صفحه‌ی ساده OK برمی‌گردونه و کاری به
# منطق ربات نداره؛ ربات جدا و به‌صورت polling کار می‌کنه.
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        pass  # لاگ‌های اضافه‌ی هر ریکوئست رو خاموش می‌کنیم
...
def start_health_server():
    ...
    thread.start()
    logger.info("وب‌سرور سلامت روی پورت %s بالا اومد.", port)
...
async def run_bot():
    ...
    # به‌جای app.run_polling() که تو پایتون 3.14 با باگ asyncio.get_event_loop
    # مواجه میشه، خودمون چرخه‌ی اجرا رو دستی مدیریت می‌کنیم.
    async with app:
        await app.start()
        await app.updater.start_polling()
        logger.info("ربات در حال اجراست...")
        try:
            await asyncio.Event().wait()  # تا ابد اجرا بمونه
        finally:
